...
Status: active at 38% after local completion of Tasks 1.1-1.4, Task 2.1, Task 2.3, and Task 2.6 through 5A-5C and 5E-5F; 5F-R stopped locally at its staged-whitespace gate before commit or external action, and an exact evidence-whitespace and provenance-normalization decision is now the sole current gate while Luxe Legacy setup, Task 2.2 / 5D, later product implementation, merge, production deployment, protected access, and final Phase 5 closeout remain separately gated
...
- **Task 2: Polish high-friction journeys without widening financial or authentication risk.** Status: active umbrella; decomposed into Tasks 2.1-2.8 after 5B intake.
  ...
  - **Task 2.4: Revise the recurring-charge surface.** Status: current Ryan decision after verified 5F-R2 release. Decide whether the surface is a cancellation watchlist, complete recurring-spend register, or combined workflow before revising its name, order, progress, labels, and undo behavior.
