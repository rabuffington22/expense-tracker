...
- **Task 3: Add CI checks that are safe for a private financial application and use only synthetic data.** The just-in-time 2026-07-25 passes separate core contract/implementation/observation from browser contract/implementation/observation so none is inferred from another. Status: active at Task 3.3.1 after Tasks 3.1-3.2 became durable without deployment through 4BJ-R and Task 3.2.1 completed through 4BK.
  ...
  - **Task 3.3: Add isolated-browser CI coverage.** Umbrella for the execution-sized action/runtime contract, local job implementation, and hosted observation below. The 4BK follow-up pass confirmed the maintained browser matrix passes locally in 286.89 seconds with temporary all-entity data, denied non-localhost browser requests, mocked external seams, both auth modes, and exact cleanup. Status: complete through 4BO; the exact repair is durable, deployed, and health verified through stopped 4BP.
...
- **Task 4: Publish and verify only explicitly approved repairs.** Status: current for a separate decision on operational Fly workflow action-runtime remediation after 4BP made the exact two-file repair durable, deployed, and health verified but stopped at a Node.js runtime warning; no remediation or Phase 4 reconciliation is authorized.
