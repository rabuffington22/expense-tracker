...
Status: Task 1P.4.1, Task 1P.4.2a, both Task 1P.4.2b migration slices, final HTMX disablement, and the first two full-page route clusters are durable, automatically deployed, and credential-free health verified through work block 4AK-R. Cash Flow and Long-Term Planning execution are durable, automatically deployed, and health verified through 4AL-R. Short-Term Planning execution is migrated and locally verified through 4AM; publication and every later page, style/document, header-enforcement, and proof slice remain separately gated.

Parent: Phase 4 Task 1P.4 / finding `P3-3J-06`.

## Purpose And Boundary

This contract defines the target browser policy, the application surfaces that must be migrated before enforcement, and the maintained proof required to close the CSP half of `P3-3J-06`. Work block 4AE created the contract without product mutation. Work block 4AF implemented the shared execution foundation, work block 4AG implemented the dashboard/report fragment slice, work block 4AH implemented the transaction and supporting-modal fragment slice, work block 4AI disabled HTMX eval and swapped-script processing after replacing three swapped inert JSON script carriers with non-script template data, work block 4AJ migrated the first full-page cluster, work block 4AK migrated categorization/upload execution, work block 4AL migrated Cash Flow and Long-Term Planning execution, and work block 4AM migrated Short-Term Planning execution into page-owned local behavior and handler-free response markup. None added a CSP header, changed authentication or Plaid behavior, or touched any live system.
...
The tracked surface contains 46 HTML templates and seven standalone document roots: the shared `base.html` shell, login, three error documents, offline, and standalone `/k/`. After local work block 4AM, the source inventory contains:
...
| Executable inline scripts | 11 | Move to maintained local static JavaScript; a nonce is not a migration substitute. Shared-shell, migrated fragment, core review-page, categorization/upload, Cash Flow, Long-Term Planning, and Short-Term Planning behavior now use local assets. |
| External executable scripts | 11 | Local theme, HTMX, app-shell, dashboard-fragment, transaction-fragment, categorization/upload, Cash Flow, Long-Term Planning, and Short-Term Planning assets plus two exact Plaid Link initializer tags. |
...
| Native inline event-handler attributes | 44 | Replace with delegated or initialized listeners before `script-src-attr 'none'`; shared-shell, migrated fragment, core review-page, categorization/upload, Cash Flow, Long-Term Planning, and Short-Term Planning handlers are removed. |
...
| Full-page execution | Migrate the remaining 11 executable inline scripts, 44 native handlers, and one full-page inert JSON carrier, including both Plaid entry pages; preserve page-specific initialization through local modules/data. | Every full-page route works with `script-src-attr 'none'` and no inline application script. | Task 1P.4.2c.1 is durable through 4AJ-R, Task 1P.4.2c.2 is durable through 4AK-R, Tasks 1P.4.2c.3a-1P.4.2c.3b are durable through 4AL-R, Task 1P.4.2c.3c is complete locally through 4AM, and Tasks 1P.4.2c.4-1P.4.2c.8 remain separate. |
...
The original eight-cluster inventory reconciled exactly to 22 executable inline scripts, 116 template-native handlers, and two full-page inert JSON carriers. The 4AM recheck additionally found and removed two Python-rendered Short-Term Planning handlers inside the same confirmed route family. After local 4AM, Tasks 1P.4.2c.4-1P.4.2c.8 reconcile to the eleven remaining executable inline scripts, forty-four template-native handlers, and one inert JSON carrier; all remain planned and unauthorized.
