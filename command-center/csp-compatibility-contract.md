...
The tracked surface contains 46 HTML templates and seven standalone document roots: the shared `base.html` shell, login, three error documents, offline, and standalone `/k/`. After local work block 4AO, the source inventory contains:

| Surface | Count | Contract consequence |
| --- | ---: | --- |
| All `<script>` elements | 22 | Separate the nine remaining inline application scripts from thirteen external executable assets. |
| Executable inline scripts | 9 | Move to maintained local static JavaScript; a nonce is not a migration substitute. Shared-shell, migrated fragment, core review-page, categorization/upload, Cash Flow, Long-Term Planning, Short-Term Planning, Weekly, Waterfall, and subscription behavior now use local assets. |
| External executable scripts | 13 | Local theme, HTMX, app-shell, dashboard-fragment, transaction-fragment, categorization/upload, Cash Flow, Long-Term Planning, Short-Term Planning, Waterfall, and subscription assets plus two exact Plaid Link initializer tags. |
...
| Native inline event-handler attributes | 16 | Replace with delegated or initialized listeners before `script-src-attr 'none'`; all completed execution slices through subscriptions are removed. |
...
| Full-page execution | Migrate the remaining 9 executable inline scripts and 16 native handlers, including both Plaid entry pages; preserve page-specific initialization through local modules/data. | Every full-page route works with `script-src-attr 'none'` and no inline application script. | Task 1P.4.2c.1 is durable through 4AJ-R, Task 1P.4.2c.2 is durable through 4AK-R, Tasks 1P.4.2c.3a-1P.4.2c.3b are durable through 4AL-R, Task 1P.4.2c.3c is durable through 4AM-R, Task 1P.4.2c.4 is durable through 4AN-R, Task 1P.4.2c.5 is complete locally through 4AO, and Tasks 1P.4.2c.6-1P.4.2c.8 remain separate. |
...
The original eight-cluster inventory reconciled exactly to 22 executable inline scripts, 116 template-native handlers, and two full-page inert JSON carriers. The 4AM recheck additionally found and removed two Python-rendered Short-Term Planning handlers inside the same confirmed route family. After local 4AO, Tasks 1P.4.2c.6-1P.4.2c.8 reconcile to the nine remaining executable inline scripts and sixteen template-native handlers with no inert JSON script carriers; all remain planned and unauthorized.
