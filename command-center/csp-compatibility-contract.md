...
| Offline and errors | Same-origin `standalone-documents.js` and `style.css`; no inline execution or style. The exact unversioned offline controller request is precached with `/offline`; delegated Retry uses one cache-disabled same-origin network-only `/health` request and accessible status feedback. | Strict core policy. The cached `/offline` response retains the header with which it was stored, so a service-worker byte or controller change must bump the Ledger cache and re-precache `/offline` plus its exact static controller without caching `/health` or protected/dynamic responses. |
...
| 1P.4.2c.8 | Offline, errors, and standalone `/k/` | 0 inline scripts; 0 source or rendered native handlers after 4AR | Blocking template-free `standalone-documents.js` preserves early theme and delegated retry across offline/errors; the offline page's exact unversioned controller request is precached so a fresh first-offline fallback can report Checking, Still offline, and Connection restored states through one network-only `/health` check. Deferred template-free `kristine.js` preserves `/k/` drill-down behavior. Maintained request and configured-auth/no-password isolated-browser proof covers exact 403/404/500 status, no exception leakage, authentication/entity boundaries, denied networking, expected synthetic status console entries only, zero unexpected browser/page errors, and exact cleanup; durable and health verified through 4AR-R with the fresh-first-offline extension implemented locally in 5P. |
