...
After 4AY, the final application-style cluster reconciles exactly to five style blocks, three template attributes, zero generated attributes, and zero runtime writes. The proven shell, transaction, categorization/upload, Cash Flow/planning, Weekly/Waterfall, Subscriptions/Payroll, Data Sources/Connected Accounts, semantic-state, bounded-percentage-class, finite-role-class, inert-geometry, Web Animations API, and browser-proof patterns are available to the remaining document-family cluster. Header generation, nonce plumbing, policy enforcement, the Plaid third-party exception, and live or publication work remain separate.
