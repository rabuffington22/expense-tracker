...
The tracked surface contains 46 HTML templates and seven standalone document roots: the shared `base.html` shell, login, three error documents, offline, and standalone `/k/`. After local work block 4AL, the source inventory contains:

| Surface | Count | Contract consequence |
| --- | ---: | --- |
| All `<script>` elements | 24 | Must distinguish local/external executable scripts from the two remaining full-page inert JSON blocks. |
| Executable inline scripts | 12 | Move to maintained local static JavaScript; a nonce is not a migration substitute. Shared-shell, migrated fragment, core review-page, categorization/upload, Cash Flow, and Long-Term Planning behavior now use local assets. |
| External executable scripts | 10 | Local theme, HTMX, app-shell, dashboard-fragment, transaction-fragment, categorization/upload, Cash Flow, and Long-Term Planning assets plus two exact Plaid Link initializer tags. |
...
| Native inline event-handler attributes | 66 | Replace with delegated or initialized listeners before `script-src-attr 'none'`; shared-shell, migrated fragment, core review-page, categorization/upload, Cash Flow, and Long-Term Planning handlers are removed. |
...
| Full-page execution | Migrate the remaining 12 executable inline scripts, 66 native handlers, and two full-page inert JSON carriers, including both Plaid entry pages; preserve page-specific initialization through local modules/data. | Every full-page route works with `script-src-attr 'none'` and no inline application script. | Task 1P.4.2c.1 is durable through 4AJ-R, Task 1P.4.2c.2 is durable through 4AK-R, Tasks 1P.4.2c.3a-1P.4.2c.3b are complete locally through 4AL, and Task 1P.4.2c.3c plus Tasks 1P.4.2c.4-1P.4.2c.8 remain separate. |
...
| 1P.4.2c.3c | Short-Term Planning | 1 inline script; 22 native handlers; 1 inert JSON carrier | Goal, budget, action-item, fetch, and dynamic transaction-edit behavior remain a separate high-interaction verification path. |
