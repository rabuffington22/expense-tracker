...
| P3-3J-05 | Medium | open | High | Mobile sidebar accessibility | At 390px, open the drawer; focus stayed on the button, background scrolled, and Escape did nothing, while focus placement/restoration, scroll lock, and Escape close are required. | Keyboard and assistive-technology navigation is incomplete. | independent-ux-repair |
| P3-3J-06 | Medium | partly-addressed | High | Session cookie and browser security policy | 4AC now makes the session cookie explicitly HttpOnly and SameSite Lax everywhere and Secure on Fly with maintained local/Fly-simulated coverage; protected HTML still lacks CSP. | Cookie transport and browser-send policy is explicit locally; compatible CSP design and publication remain separate. | finish-csp-separately-after-cookie-contract |
| P3-3J-C01 | Medium | coverage | High | Auth, CSRF, redirect, cache, public fields, manifest, browser, responsive behavior | 4A added maintained request/cache contracts, but isolated-browser, manifest/icon, exact public-field, and responsive-navigation coverage remains. | The repaired boundary and adjacent browser behavior are only partly guarded. | pair-browser-coverage-with-3J-decisions-and-repairs |
