...
| P3-3J-06 | Medium | partly-addressed | High | Session cookie and browser security policy | 4AC makes the session cookie explicit, 4AE freezes a strict core/narrow Plaid CSP contract, 4AF completes the shared execution foundation, and 4AG-R makes the dashboard/report fragment slice durable and deployed; protected HTML still lacks CSP. | Cookie policy, planning, shared execution, and dashboard/report fragments are durable; transaction/modal fragments, final HTMX disablement, page/style/header work, and final production proof remain separate. | confirm-transaction-fragment-slice-after-4ag-r |
