...
| P3-3J-06 | Medium | partly-addressed | High | Session cookie and browser security policy | 4AC makes the session cookie explicit, 4AE freezes a strict core/narrow Plaid CSP contract, 4AF completes the shared execution foundation, and 4AG completes the dashboard/report fragment slice locally; protected HTML still lacks CSP. | Cookie policy, planning, and shared execution are durable; dashboard/report fragments are locally verified; transaction/modal fragments, final HTMX disablement, page/style/header work, publication, and production proof remain separate. | confirm-transaction-fragment-slice-after-4ag |
