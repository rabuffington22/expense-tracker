...
from core.sync_coordination import SyncLease, try_acquire_sync_lease
...
def _background_sync(lease: SyncLease):
    """Sync Personal and LL through the maintained core, then release the lease."""
    try:
        from web.routes.plaid import _sync_entity

        for entity_key in ("personal", "luxelegacy"):
            try:
                init_db(entity_key)
                result = _sync_entity(entity_key)
                if result.get("errors"):
                    log.warning("Kristine sync completed with errors for %s", entity_key)
            except Exception:
                log.warning("Kristine sync failed for entity %s", entity_key)
...
def _start_background_sync() -> bool:
    """Start a throttled worker only after acquiring the shared sync lease."""
    import time as _time
    from web.routes.plaid import _plaid_available
    ...
    with _launch_lock:
        ...
        worker = threading.Thread(target=_background_sync, args=(lease,), daemon=True)
        try:
            worker.start()
        except Exception:
            lease.close()
...
# Transfer/income categories excluded from budget actuals (from single source of truth)
...
def _get_focus_budget(conn, month: str) -> list[dict]:
    ...
    sub_rows = conn.execute(
        f"WITH {_cte} "
        "SELECT COALESCE(NULLIF(t.category,''),'Uncategorized') as cat, "
        "COALESCE(NULLIF(t.subcategory,''),'General') as subcat, "
        "ABS(SUM(t.amount)) as total "
        "FROM t "
        "WHERE strftime('%%Y-%%m', t.date) = ? "
        "AND t.amount < 0 "
    ...
    txn_rows = conn.execute(
        f"WITH {_cte} "
        "SELECT t.date, t.merchant_canonical, t.description_raw, "
        "ABS(t.amount) as amt, t.category, t.subcategory "
        "FROM t "
        "WHERE strftime('%%Y-%%m', t.date) = ? "
        "AND t.amount < 0 "
        "AND t.category IN (%s) "
        "ORDER BY t.date DESC" % cat_placeholders,
        (month, *focus_cats),
    ).fetchall()
...
def _get_ll_summary(conn, month: str) -> dict:
    ...
    income_row = conn.execute(
        f"WITH {_cte} "
        "SELECT COALESCE(SUM(t.amount), 0) as total FROM t "
        "WHERE strftime('%Y-%m', t.date) = ? AND t.amount > 0 "
        "AND COALESCE(t.category,'') NOT IN ('Internal Transfer', 'Credit Card Payment', 'Owner Contribution')",
        (month,),
    ).fetchone()
    ...
    expense_row = conn.execute(
        f"WITH {_cte} "
        "SELECT COALESCE(SUM(ABS(t.amount)), 0) as total FROM t "
        "WHERE strftime('%Y-%m', t.date) = ? AND t.amount < 0 "
        "AND COALESCE(t.category,'') NOT IN ('Internal Transfer', 'Credit Card Payment', 'Owner Contribution')",
        (month,),
    ).fetchone()
    ...
    # Owner Contribution (money moved from LL to personal — positive signal!)
    draw_row = conn.execute(
        f"WITH {_cte} "
        "SELECT COALESCE(SUM(ABS(t.amount)), 0) as total FROM t "
        "WHERE strftime('%Y-%m', t.date) = ? AND t.category = 'Owner Contribution'",
        (month,),
    ).fetchone()
