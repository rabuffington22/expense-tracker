...
10. **Card flip animation on Cash Flow + Planning** — Clicking a card triggers a 3D flip revealing the suit back (♦ green for bank/asset, ♠ purple for credit/liability), then the modal scales in from center with spring easing (`modalScaleIn` keyframe: scale 0.4→1, translateY 20→0). Closing the modal flips the card back. Shared `.card-flip-inner`/`.card-flip-front`/`.card-flip-back` CSS structure. `overflow: hidden` → `overflow: visible` on `.cf-box` and `.pl-box` (required for `preserve-3d`). JS: `cfFlipAndOpen()`/`plFlipAndOpen()` add `card-flipped` class → 350ms delay → open modal. Close removes class to flip back. Modal scrim fades in via `modalScrimIn` keyframe.
